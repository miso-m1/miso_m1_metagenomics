...
dir.create(here("figures", "dataPrep"), recursive = TRUE, showWarnings = FALSE)
```
...
bp_seq_depth <- ggplot(data.frame(depth = sample_depth), aes(x = depth)) +
  ...
  labs(title = "Distribution of Sequencing Depth",
       ...
       y = "Frequency")
bp_seq_depth
ggsave(here("figures", "dataPrep", "sample_sum.pdf"), bp_seq_depth)
```
...
Tax_phyllum
ggsave(here("figures", "dataPrep", "Tax_phyllum.pdf"), Tax_phyllum)
```
...
Tax_disease_group
ggsave(here("figures", "dataPrep", "Tax_disease_group.pdf"), Tax_disease_group)
```
...
Top10b_disease
ggsave(here("figures", "dataPrep", "Top10b_disease.pdf"), Top10b_disease)
```
...
Sample_dist_gender <- ggplot(gender_counts, aes(x = gender, y = Count, fill = gender)) +
  ...
  labs(title = "Number of Samples by Gender",
       ...
       y = "Number of Samples")
Sample_dist_gender
ggsave(here("figures", "dataPrep", "Sample_dist_gender.pdf"), Sample_dist_gender)
```
...
ggsave(here("figures", "dataPrep", "SampleF_dist_disease.pdf"), SampleF_dist_disease)
```
...
ggsave(here("figures", "dataPrep", "SampleM_dist_disease.pdf"), SampleM_dist_disease)
```
...
Sample_dist_country<- ggplot(country_counts, aes(x = country, y = Count, fill = country)) +
  ...
  labs(title = "Number of Samples by Country",
       ...
       y = "Number of Samples")
Sample_dist_country
ggsave(here("figures", "dataPrep", "Sample_dist_country.pdf"), Sample_dist_country)
```
...
ggsave(here("figures", "dataPrep", "Sample_dist_country_disease.pdf"), Sample_dist_country_disease)
```
...
ggsave(here("figures", "dataPrep", "B10class_dist_gender.pdf"), B10class_dist_gender)
```
...
Bclass_dist_gender <- plot_bar(metagenomics_class, x = "gender", fill = "Class") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
Bclass_dist_gender
ggsave(here("figures", "dataPrep", "Bclass_dist_gender.pdf"), Bclass_dist_gender)
```
...
save(metagenomics, B10class_dist_gender, Bclass_dist_gender, bp_seq_depth, Sample_dist_gender, SampleF_dist_disease, SampleM_dist_disease, Tax_disease_group, Tax_phyllum, Top10b_disease, Sample_dist_country, Sample_dist_country_disease, file = here::here("posts", "datapreparation.RData"))
```
