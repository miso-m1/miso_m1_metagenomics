...
```{r}
diseases<- c("ibd ulcerative colitis", "t2d", "cirrhosis", "cancer")
disease_counts <- meta_df %>%
...
Number of samples of diseases for China:
...
SampleC_dist_disease <- ggplot(disease_counts %>% filter(country == "china"),
       aes(x = disease, y = Count, fill = disease)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Number of Samples from China per Disease") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
SampleC_dist_disease
```

Number of samples of diseases for France:
...
SampleFr_dist_disease <- ggplot(disease_counts %>% filter(country == "france"),
       aes(x = disease, y = Count, fill = disease)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Number of Samples from France per Disease") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
SampleFr_dist_disease
```

Number of samples of diseases for Spain:
