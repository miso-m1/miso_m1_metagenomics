...
The `phyloseq` package in R already has built in functions to calculate these metrics for data, but with raw counts required (whole numbers ≥0).

`alpha_div <- estimate_richness(metagenomics, measures = c("Shannon", "Simpson", "InvSimpson", "Observed"))`

## Alpha Diversity Formulas

The `phyloseq` package in R already has built in functions to calculate the different metrics, but for data with required **raw counts** (whole numbers, ≥0).
