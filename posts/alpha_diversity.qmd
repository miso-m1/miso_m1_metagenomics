...
dir.create(here("figures", "Alpha"), recursive = TRUE, showWarnings = FALSE)
```
...
$\lambda$ = $\sum_{i=1}^{R} p_i^2$ (the probability that two individuals taken from the sample at random represent the same species). Therefore, $1-\lambda$ is the probability that two individuals taken from the sample at random represent different species.
...
Number of unique taxa (OTUs) present in each.
:::
