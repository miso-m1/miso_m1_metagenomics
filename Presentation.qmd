...
```{r}
df_bc <- as.data.frame(Tuckey_BC[[1]])
df_jaccard <- as.data.frame(Tuckey_Jaccard[[1]])

df_bc$Comparison <- rownames(df_bc)
df_bc$Metric <- "Bray-Curtis"

df_jaccard$Comparison <- rownames(df_jaccard)
df_jaccard$Metric <- "Jaccard"

combined_tukey <- rbind(df_bc, df_jaccard)

tukey_sorted <- combined_tukey[order(combined_tukey$`p adj`, decreasing = TRUE), ]

tukey_extremes <- rbind(head(tukey_sorted, 2), tail(tukey_sorted, 2))

tukey_final <- tukey_extremes[, c("Metric", "Comparison", "diff", "lwr", "upr", "p adj")]

knitr::kable(tukey_final, 
             digits = 4, 
             row.names = FALSE, 
             col.names = c("Metric", "Comparison", "Difference", "Lower CI", "Upper CI", "P-value (adj)"))
```

➔ t2d and ibd_ulcerative_colitis seems to have very differing dispersion variances, causing the heterogeneity
