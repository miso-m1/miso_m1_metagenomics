---
title: "Consensus Network Inference in Gut Metagenomics"
format: html
editot: visual
---
...
# Function to generate and save the multicolored global network
export_global_network <- function(network_list, tax_level, nom_fichier) {
  ...
  color_palette <- setNames(rainbow(length(unique_diseases)), unique_diseases)
  ...
  # High-definition file export (300 DPI PNG)
  chemin <- file.path("../graph/Network", nom_fichier)
  png(filename = chemin, width = 2500, height = 2500, res = 300)
  
  plot(master_net, layout = layout_with_fr(master_net), vertex.size = v_size,
       vertex.color = "lightgrey", vertex.frame.color = "white",
...
dossier_export <- "../graph/Network"
if(!dir.exists(dossier_export)) dir.create(dossier_export, recursive = TRUE)
```
