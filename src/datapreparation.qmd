---
  title: "Data preparation for Statistical Analysis of Metagenomic Data"
  format: html
  editor: visual
---
...
sample_depth <- sample_sums(carbom)
...
```{r}
sample_sums(carbom)[sample_sums(carbom) < 90]
```
...
```{r}
carbom_class <- tax_glom(carbom, taxrank = "Class")
carbom_class_per <- transform_sample_counts(carbom_class, function(x) x / sum(x))
top_classes <- names(sort(taxa_sums(carbom_class_per), decreasing = TRUE))[1:10]
carbom_class_top <- prune_taxa(top_classes, carbom_class_per)

plot_bar(carbom_class_top, x = "gender", fill = "Class") +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
...
plot_bar(carbom_class, x = "gender", fill = "Class") +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
